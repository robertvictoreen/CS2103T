...
**Chosen Implementation:** Only sorts by names of persons in the Address Book. +
...
**Alternative :** Sorts persons in the Address Book by any field (e.g. sorting by email or tag), which is given as an input by the user. +
...
**Cons:** +
Need to ensure that the image is not deleted, or it won't show. +

**Aspect:** Putting a limit on profile picture size +

**Chosen Implementation:** +
Set a restriction of 5MB for each picture. +
**Pros:** +
Prevents a  user from crashing the app by putting an image that's too large in size. +
**Cons:** +
Some pictures may be larger than 5MB. They can use image compression services to compress it in order to fulfill the limit.

**Alternative:** +
Not put a specified restriction to the size of profile photo +
**Pros:** +
User can upload image of any size. +
**Cons:** +
Performance of the app might be unstable if it contains too many large images.
...
==== Current Implementation
We implemented a feature which allows a user to delete the profile photo and change back to default.
...
**Pros:** +
Easier if developer decides to change the path of the default photo, or the properties of the ProfilePhotoclass +
**Cons:** +
Creating a new object each time might be highly memory-consuming, but probably insignificant on a modern PC. +
...
===== Aspect: Data Structure to support assignment operations
* **Alternative 1 (current choice):** Adaptation of Assignment model, using a hashmap for each student to record the attendance/absense
