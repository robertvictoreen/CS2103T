...
the user, unless the user ends the text with `!` or `?`. When text is added to a `Note` that already has text, the
current `text` is changed to end with a comma (`,`) instead, before the new text is appended to the end.
// add activity diag here to show ^
** Pros: +
