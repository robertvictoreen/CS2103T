...
==== Current Implementation
* `Note` texts are automatically ended with a full stop (`.`) if it is not added by the user, unless the user ends the text with `!` or `?`. When text is added to a `Note` that already has text, the current `text` is changed to end with a comma (`,`) instead, before the new text is appended to the end.
** Pros: +
...
* **Alternative 1:** No editing of `Note` text, `text` will be whatever the user inputs.
** Pros: +
...
==== Edit Note Feature
* `editnote` command currently uses the `note` and `delnote` commands to execute.
...
** Pros: +
*** Easy to implement.
*** Adapts with any changes to the `NoteCommand` and `DeleteCommand` without needing to update.
*** Reduces amount of repeated code.
** Cons: +
*** Some overhead accrued due to `NoteCommand` and `DeleteNoteCommand` object creation.
*** Dependencies between `EditNoteCommand` and both `NoteCommand` and `DeleteNoteCommand`.
//end:note[]
