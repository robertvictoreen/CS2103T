...
.. Test case: `note 1 test` +
   Expected: If the first student had no note, his note as displayed at the bottom right is now `test.`, else, `test.` is added to back of his existing
   note, with the full-stop of existing note changed to a comma. Details of student whom the note was added to shown in
   the status message. Timestamp in the status bar is updated.
