...
[NOTE]
====
Undoable commands: those commands that modify the student list's content (`add`, `delete`, `edit`, `markAssignment`, `markAttendance`, `note`, `delnote`, `editnote`, `sort` and `clear`).
====
